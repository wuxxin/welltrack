...
- make a new prototype in `src/prototype/new-pain-entry.html`:
...
- Gui Dark Theme:
    ...
    - while making the theme, go through any html generation, look if there are always the same of nearly the same class overrides to simplify logic and usage. look for any css that is not used or could be cleaned out before implementing it in dark mode. go over any css and think if it needs to change for dark mode. correlate background (blackish) with foreground whiteish, maintain a good contrast like in the bright theme. make sure most backgrounds in dark mode are blackish or darkish, but not bright. adjust the content inside of these containers. explain every color in welltrack and if its subjected to theme change for dark theme. the darkmode should cover also button background and foreground colors. try to keep the finde zebra cycling in a few lists that visually aid the eye.
...
---
protokoll: mood matrix is not visible from background both on dark and bright theme.
"Montag 13.10.2025" on today should be brighter like "Heute" on the same page.
event container bg-gray-100 in it, so event entries have bright background.
cleanup all inline class related coloring, or be sure to comment the reason for usage.
manage event list has bg-grey bg-white as diffference coloring, still on the black theme.
think of solutions for any two colored entries coloring that works with dark and bright theme.
all submenu buttons lost their visual feedback when pressed, when active and on hovering over. this is neither visible in the bright nor in the dark theme now. maybe traces are sometimes visible. restore the coloring of the submenu buttons like before (with maybe new implementation), and make it work for the dark theme too.
also: some buttons are misslabeld: correct typing: DANGER: most dangerous items are colored in brighter red. This include: on settings/import-export/delete-all data: the "Löschen" button in the modal is that, because it actually deletes the data. next is import-export/import data:"Daten überschreiben" , next is on eventtype delete modal the button "Löschen". next is "POTENTIAL-DANGER-AHEAD": settings:data import, settings:data import: "Daten Vereinen" because it could potentially go wrong. The abort "Abbrechen" buttons should always have the theme dark or bright background. "Speichern", "Neue hinzufügen", "Browser Benachrichtigungen erlauben", sollten im grund theme sein.
bei ansonsten neutraler farbwahl sollte gelegentlich bei akzenten ein dezentes theme hell lila  verwendet werden, wie im ursprünglichen theme.
