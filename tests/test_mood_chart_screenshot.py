...
def test_mood_chart_screenshots(page: Page, live_server):
    ...
    with open('tests/fixtures/sample-data-mood-chart.json', 'r') as f:
    ...
    page.screenshot(path="build/screenshots/mood_chart_7_days.png", full_page=True)

    # Take screenshot for 28 days view
    page.locator("#time-range-selector").select_option("28")
    time.sleep(1) # wait for re-render
    page.screenshot(path="build/screenshots/mood_chart_28_days.png", full_page=True)

    # Take screenshot for 84 days view
    page.locator("#time-range-selector").select_option("84")
    time.sleep(1) # wait for re-render
    page.screenshot(path="build/screenshots/mood_chart_84_days.png", full_page=True)